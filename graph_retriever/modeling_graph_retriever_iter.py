...
from modules.bert_iter_models import IterBertModelForRetrieval, IterBertPreTrainedConfig, IterBertModelForRetrievalV2, \
    IterBertModelForRetrievalV3

try:
    # from graph_retriever.utils import tokenize_question
    # from graph_retriever.utils import tokenize_paragraph
    from graph_retriever.utils import expand_links, GraphRetrieverConfig
except ImportError:
    # from utils import tokenize_question
    # from utils import tokenize_paragraph
    from utils import expand_links, GraphRetrieverConfig


class BertForGraphRetriever(IterBertModelForRetrieval):

    def __init__(self, config: IterBertPreTrainedConfig, graph_retriever_config: GraphRetrieverConfig):
    ...
    def encode(self, input_ids, token_type_ids, attention_mask, split_chunk=None):
        ...
        if not self.graph_retriever_config.disable_rnn_layer_norm:
                    ...
                    if len(new_titles) > 0:
                        ...
                        for linked_title in new_titles:
                            ...
                            tokenized_outputs = tokenizer(
                                text=examples[i].question,
                                text_pair=linked_para,
                                padding='max_length',
                                truncation='only_second',
                                max_length=self.graph_retriever_config.max_seq_length
                            )
                            input_ids_ = tokenized_outputs["input_ids"]
                            input_masks_ = tokenized_outputs["attention_mask"]
                            if "token_type_ids" in tokenized_outputs:
                                segment_ids_ = tokenized_outputs["token_type_ids"]
                            else:
                                segment_ids_ = [0] * len(input_ids_)

                            # input_ids_, input_masks_, segment_ids_ = tokenize_paragraph(linked_para, tokens_q,
                    ...
                    for b in range(beam):
                        ...
                        if self.graph_retriever_config.pruning_by_links:
                            if pred_[b][0][-1] == eos_index:
                                ...
                                for k in range(len(examples[i].title_order)):
                                    if k not in pred_[b][1] and examples[i].title_order[k] not in \
                                            examples[i].all_linked_paras_dic[examples[i].title_order[pred_[b][0][-1]]]:
                                        output[b, :, k] = 0.0
...
class BertForGraphRetrieverV2(IterBertModelForRetrievalV2):
    def __init__(self, config: IterBertPreTrainedConfig, graph_retriever_config: GraphRetrieverConfig):
...
class BertForGraphRetrieverV3(IterBertModelForRetrievalV3):
