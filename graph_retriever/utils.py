...
class GraphRetrieverConfig:
    ...
    def __init__(self,
        ...
        assert 1 <= self.min_select_num <= self.max_select_num
...
def tokenize_question(question, tokenizer: PreTrainedTokenizer):
    raise NotImplementedError

    # tokens_q = tokenizer.tokenize(question)
    # tokens_q = [tokenizer.cls_token] + tokens_q + [tokenizer.sep_token]
    #
    # return tokens_q
...
def tokenize_paragraph_transformers(passage, question, max_seq_length, tokenizer: PreTrainedTokenizer):
...
def convert_examples_to_features(examples, max_seq_length, max_para_num, graph_retriever_config,
    ...
    for (ex_index, example) in enumerate(tqdm(examples, desc='Example', dynamic_ncols=True)):
        ...
        output_masks = [([1.0] * len(input_ids) + [0.0] * (max_para_num - len(input_ids) + 1)) for _ in
        ...
        for redundant_gold in example.all_redundant_gold:
            ...
            output_masks_r = [([1.0] * len(input_ids_r) + [0.0] * (max_para_num - len(input_ids_r) + 1)) for _ in
                              range(max_para_num + 2)]
